...
class OSPFswitch_13(topo_switch_13.TopoSwitch_13):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #                            'dst2':[...], 'dst3':[...]...}
        #                 'switch2':{...},'switch3':{...}...}
        # flow_set_next = {'switch1':{'dst1':[{'eth_src':eth_src1,'eth_dst':eth_dst1,'tcp_dst':tcp_dst1,'next':next1},
        #                                        {'eth_src':eth_src2,'eth_dst':eth_dst2,'tcp_dst':tcp_dst2,'next':next2}...],
        #                                'dst2':[...], 'dst3':[...]...}
        #                     'switch2':{...},'switch3':{...}...}
        self.flow_set = {}
        self.flow_set_dst = {}
    ...
    def _packet_in_handler(self, ev):
        ...
        tcp_pro = pkt.get_protocols(tcp.tcp)
        tcp_dst = 0
        tcp_src = 0

        if len(tcp_pro) != 0:
            tcp_pro = tcp_pro[0]
            tcp_dst = tcp_pro.dst_port
            tcp_src = tcp_pro.src_port
        ...
        if paths is not None and len(paths) > 0:
            out_port = self.install_path(paths,src_mac,dst_mac,type_mac,dpid,parser,ofproto,msg,tcp_dst)
        else:
    ...
    def install_path(self,paths,src,dst,type,in_dpid,parser,ofproto,msg,tcp_dst=0):
            ...
            if tcp_dst != 0:
                ...
                if tcp_dst >= 0:
                    ...
                    if item not in self.flow_set[switch]:
                        self.flow_set[switch].append(item)
                    self.flow_set_dst.setdefault(switch, {})
                    self.flow_set_dst[switch].setdefault(dst, [])
                    if item not in self.flow_set_dst[switch][dst]:
                        self.flow_set_dst[switch][dst].append(item)
                    item = {'eth_src':src,'eth_dst':dst,'tcp_dst':tcp_dst,'next':node2}
                    self.flow_set_next.setdefault(switch, {})
                    self.flow_set_next[switch].setdefault(dst, [])
                    if item not in self.flow_set_next[switch][dst]:
                        self.flow_set_next[switch][dst].append(item)
                target_match = parser.OFPMatch(eth_src=src,eth_dst=dst,eth_type=type,ip_proto=6,tcp_dst=tcp_dst)
            else:
                target_match = parser.OFPMatch(eth_src=src,eth_dst=dst,eth_type=type,ip_proto=1)
